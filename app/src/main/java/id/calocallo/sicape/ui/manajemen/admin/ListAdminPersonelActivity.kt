...
class ListAdminPersonelActivity : BaseActivity() {
    ...
    private var adapterAdmin = ReusableAdapter<UserCreatorResp>(this)
    private lateinit var callbackAdmin: AdapterCallback<UserCreatorResp>
    ...
    private fun getListAdmin() {
        rl_pb.visible()
        NetworkConfig().getService().getListAdmin("Bearer ${sessionManager1.fetchAuthToken()}")
    ...
    private fun listAdmin(list: ArrayList<UserCreatorResp>?) {
        callbackAdmin = object :AdapterCallback<UserCreatorResp>{
            override fun initComponent(itemView: View, data: UserCreatorResp, itemIndex: Int) {
            ...
            override fun onItemClicked(itemView: View, data: UserCreatorResp, itemIndex: Int) {
                val intent = Intent(this@ListAdminPersonelActivity, DetailAdminPersonelActivity::class.java)
